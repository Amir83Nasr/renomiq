...
import { Search, FileText, AlertCircle, Minus, File } from 'lucide-react';
...
export function CompactFileList({
  ...
  const extensions = useMemo(() => {
    ...
    files.forEach((f) => extSet.add(f.extension.toLowerCase()));
  ...
  const someFilteredSelected =
    selectedFilteredCount > 0 && selectedFilteredCount < filteredFiles.length;
    ...
    <div className="space-y-3">
      ...
      <div className="flex flex-col sm:flex-row gap-2">
        ...
        <select
          ...
          className="px-3 py-1.5 rounded-md border border-input bg-background text-sm h-9"
        >
          <option value="all">
